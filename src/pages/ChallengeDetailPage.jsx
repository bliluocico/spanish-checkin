          ...
          <motion.div
            ...
            className={`rounded-3xl p-6 text-center ${
              challenge.winner_id === user.id
                ? 'bg-[#FFF9C4] border-2 border-[#FFD93D]'
                : challenge.failed_user_id === user.id
                ? 'bg-[#FFEBEE] border-2 border-[#EF9A9A]'
                : 'bg-[#E8F5E9] border-2 border-[#A5D6A7]'
            }`}
          >
            <div className="text-5xl mb-2">
              {challenge.winner_id === user.id ? '🏆' : challenge.failed_user_id === user.id ? '💔' : '🤝'}
            </div>
            <p className="text-xl font-extrabold text-[#4A3728]">
              {challenge.winner_id === user.id ? '恭喜获胜！' : challenge.failed_user_id === user.id ? '挑战失败' : '握手言和'}
            </p>
            <p className="text-sm text-[#8B7355] mt-1">{challenge.description || '挑战已结束'}</p>
          </motion.div>
        )}
